      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-4 flex items-baseline gap-3">
            <span className="text-3xl font-bold">{formatCurrency(product.displayPrice, product.currency)}</span>
          ...
          <div>
            ...
            <div className="mt-3 flex flex-col gap-2 sm:flex-row">
              {product.marketplaces.length > 0 ? (
                product.marketplaces.map((link) => <AffiliateCtaButton key={link.marketplace} link={link} />)
              ) : (
                <p className="text-sm text-muted-foreground">No marketplace links yet.</p>
              )}
            </div>
            <p className="mt-3 text-xs text-muted-foreground">
              We may earn a commission if you make a purchase through these links, at no extra cost to you. See our{" "}
